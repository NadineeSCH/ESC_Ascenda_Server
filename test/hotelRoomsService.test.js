...
describe("hotelDetailsService", () => {
    ...
    rooms: [
      {
        ...
        price: 4772.05,
        images: [{ hero_image: true, url: "https://example.com/room1.jpg" }],
      },
    ],
  };
  ...
  describe("successful responses", () => {
    ...
    it("should handle room without hero image", async () => {
      const responseWithoutHero = JSON.parse(JSON.stringify(mockPollerResponse));
  ...
  describe("error handling", () => {
    it("should return error when API request fails", async () => {
      ...
      expect(result).toEqual({
        success: false,
        error: "Failed to fetch from external API: API timeout",
    ...
    it("should handle missing rooms in response", async () => {
      fetchMock.mockResponseOnce(JSON.stringify({ completed: true }));
