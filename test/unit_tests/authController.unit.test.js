...
describe("authController - Unit Tests", () => {
  ...
  describe("tempUsers Map Tests", () => {
    beforeEach(() => {
      // Clear any existing tempUsers before each test
      authController.tempUsers.clear();
    });

    it("should cleanup expired users using performCleaning", () => {
      ...
      // Add user with expiry time BEFORE advancing timers
      const expiryTime = Date.now() - 1000; // 5 minutes from now
      authController.tempUsers.set(mockUser.email, {
        ...mockUser,
        expiresAt: expiryTime,
      });

      // Verify user is initially there
      expect(authController.tempUsers.has(mockUser.email)).toBe(true);

      //perform cleaning
      authController.performCleaning();
      
      // Verify user is cleaned up
      expect(authController.tempUsers.has(mockUser.email)).toBe(false);

    });

    it("should NOT cleanup non-expired users", () => {
      ...
      // Add user that expires in 10 minutes
      authController.tempUsers.set(mockUser.email, {
        ...mockUser,
        expiresAt: Date.now() + 300000, // 10 minutes from now
      });
  ...
  describe("authController.signup Unit Tests", () => {
    ...
    it("should successfully sign up a user and send OTP", async () => {
      ...
      expect(authController.tempUsers.get(req.body.email)).toEqual(
